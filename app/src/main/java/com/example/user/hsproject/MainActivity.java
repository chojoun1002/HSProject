...
/**
 * Created by on 2015-12-30.
 *
 * ViewPager를 활용한 리스트뷰를 구현.
 *
 ...
 * 사이드메뉴 구현
 * 네트워크 통신 여부 확인 가능
 * 다이얼 로그를 사용한 이미지 상세보기 가능
 * 횡스크롤 이동을 제한
 * 버튼을 이용한 뷰페이지 이동 가능
 * 네비게이션바 메튜 선택시 뷰페이지 이동 가능
 * 이미지 투명화 및 페이딩 가능
 * 이미지 선택 후 다이얼 로그에서 핀치줌으로 확대 가능
 *  (photoview 라이브러리를 추가하여 이미지 핀치줌이 가능하도록 구현함)
 * 글라이드사용 이미지 추가 및 썸네일 이미지 원형으로 처리
 * 백버튼 2번 클릭시 종료 가능
 *
 */
public class MainActivity extends AppCompatActivity
    ...
    // viewPager 사용을 위한 view 정의
    View viewPager;

    //각 메뉴의 아이콘
    ImageView menu_icon;

    //핀치줌사용을 위한 PhotoViewAttacher 선언
    PhotoViewAttacher mAttacher;

    // 데이터 통신을 위한 OkHttpClient 구현
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // 메뉴 아이콘의 투명도 여부 설정을 위해 정의. [온라인팀 허용]
        ...
        // 나의 프로필 사진 터치시에 프로필 사진 다이얼로그로 출력 [온라인팀 허용]
        ...
        myPic.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                dialog.setTitle("My Profile Image");
    ...
    // PagerAdapterClass 정의. [물류정보팀 조준열]
        ...
        public Object instantiateItem(final ViewGroup pager, int position) {
            ...
            if (position == 0) {
                ...
                if (icon.length() > 1) {
                    ...
                    // 상세사진 클릭시 별도의 다이얼로그 나타내기 + 핀치줌 적용 [물류정보팀 조준열],[온라인팀 손동현], [온라인팀 허용]
    ...
    // Back 버튼이 눌리 었을때 작동 하는 이벤트 처리 모음. [온라인팀 손동현]
    @Override
    ...
    // 네비게이션 바 메뉴 클릭시 발생하는 이벤트. [온라인팀 손동현]
    ...
    // 메뉴에 따른 메뉴 아이콘 변경 (페이드 처리) [온라인팀 허용]
    ...
    // 인터넷 또는 WIFI가 연결되어 있는지 확인 [온라인팀 허용]
